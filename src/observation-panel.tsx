...
function Confidence({observation}:{observation:Observation}){const width=`${Math.round(observation.confidence*100)}%`;return <div className="observation-confidence"><div><span>SÄKERHET</span><strong>{observation.confidenceLabel}</strong></div><div className="confidence-track"><i style={{width}}/></div><small>{Math.round(observation.confidence*100)}% · baserat på {observation.evidenceCount} dagar</small></div>}

function ObservationPanel(){
 const [stories,setStories]=useState<DailyStory[]>(readStories),[selected,setSelected]=useState<Observation|null>(null);
 ...
 useEffect(()=>{const refresh=()=>setStories(readStories());window.addEventListener('storage',refresh);window.addEventListener('liv:daily-stories-changed',refresh as EventListener);const timer=window.setInterval(refresh,1200);return()=>{window.removeEventListener('storage',refresh);window.removeEventListener('liv:daily-stories-changed',refresh as EventListener);window.clearInterval(timer)}},[]);
 return <section className="observation-section" aria-labelledby="observation-title">
   <header className="observation-heading"><span>01</span><div><p>AKTUELLA OBSERVATIONER</p><h2 id="observation-title">Det som återkommer.</h2><small>Varje observation måste kunna öppnas och förstås genom familjens egna anteckningar.</small></div></header>
   {stories.length<3?<div className="observation-empty"><strong>Vi behöver lite mer tid tillsammans.</strong><p>När minst tre dagar har sparats kan Liv börja visa försiktiga, spårbara observationer.</p><span>{stories.length} av 3 dagar sparade</span></div>:observations.length===0?<div className="observation-empty"><strong>Inget tydligt mönster ännu.</strong><p>Det är också värdefull information. Liv väntar hellre än att dra en för snabb slutsats.</p></div>:<div className="observation-grid">{observations.map(observation=><article className="observation-card" key={observation.id}><div className="observation-icon">{observation.icon}</div><h3>{observation.title}</h3><p>{observation.explanation}</p><Confidence observation={observation}/><button onClick={()=>setSelected(observation)}>Visa underlag →</button></article>)}</div>}
...
if(host){ReactDOM.createRoot(host).render(<React.StrictMode><ObservationPanel/></React.StrictMode>);const place=()=>{const target=document.getElementById('learning-workspace-content');if(target&&host.parentElement!==target)target.appendChild(host)};new MutationObserver(place).observe(document.body,{childList:true,subtree:true});place()}
