...
export function DaysTimeline(){
 ...
 return <div className="days-workspace-surface">
   <div className="days-tools"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Sök i berättelser, minnen och taggar…"/><select value={mood} onChange={e=>setMood(e.target.value as 'Alla'|Mood)}><option value="Alla">Alla känslor</option><option>🙂</option><option>😐</option><option>☹️</option></select><select value={energy} onChange={e=>setEnergy(e.target.value as 'Alla'|Energy)}><option value="Alla">Alla energinivåer</option><option>Låg</option><option>Normal</option><option>Hög</option></select><strong>{filtered.length} dagar</strong></div>
