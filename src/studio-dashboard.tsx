...
export function StudioDashboard({navigate}:Props){
 const[workspace,setWorkspace]=useState<'dashboard'|'people'|'memories'>('dashboard');
 const cards=useMemo(()=>[
  ...
  {id:'knowledge' as const,n:'05',title:'Kunskap',count:count('viktors-liv.knowledge.v1'),unit:'förklaringar',copy:'Bevara vad familjen vet och vad det betyder för Viktor.',state:'Arbetsyta redo'},
 ...
 const open=(destination:typeof cards[number]['id'])=>destination==='people'?setWorkspace('people'):destination==='memories'?setWorkspace('memories'):navigate(destination);
 return <main className="studio-page">
  ...
  <section className="studio-status"><article><b>{count('liv.memories.v1')}</b><span>Minnen med betydelse</span></article><article><b>{count('viktors-liv.people.v1')}</b><span>Människor runt Viktor</span></article><article><b>{count('viktors-liv.documents.v1')}</b><span>Dokument med sammanhang</span></article><article><b>{count('viktors-liv.journey.v1')}</b><span>Bevarade livshändelser</span></article></section>
