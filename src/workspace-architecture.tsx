...
type EconomyModel={budget:Budget;additional:MoneyItem[];version:1};
...
function readEconomyModel():EconomyModel{
  const existing=readJson<EconomyModel|null>(modelKey,null);
  if(existing?.version===1)return{version:1,budget:{...defaultBudget,...existing.budget},additional:Array.isArray(existing.additional)?existing.additional:[]};
  ...
  const additional=previousItems.filter(item=>!legacyDefaultIds.has(item.id)&&!['Aktivitetsersättning','Boende och vardag','Resestöd'].includes(item.title));
  return{version:1,budget:previousBudget,additional};
}
...
function EconomyWorkspace(){
  ...
  const extraIncome=model.additional.filter(item=>item.kind!=='Utgift').reduce((sum,item)=>sum+item.amount,0);
  const extraExpense=model.additional.filter(item=>item.kind==='Utgift').reduce((sum,item)=>sum+item.amount,0);
  const baseIncome=budget.activity+budget.childBenefit;
  ...
  const baseLiving=household+personal;
  const income=baseIncome+extraIncome;
  const living=baseLiving+extraExpense;
  ...
  const add=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=event.currentTarget;const data=new FormData(form);const title=String(data.get('title')||'').trim();if(!title)return;const item:MoneyItem={id:`money-${Date.now()}`,title,amount:Math.max(0,Number(data.get('amount')||0)),kind:String(data.get('kind')||'Utgift') as MoneyItem['kind'],date:String(data.get('date')||''),note:String(data.get('note')||'')};setModel(current=>({...current,additional:[...current.additional,item]}));form.reset();setFormOpen(false)};
  ...
  const linkedRows:MoneyItem[]=[
    ...
    {id:'linked-household',title:'Hushållsbidrag',amount:household,kind:'Utgift',date:'Månadsvis',note:'Mat, boende och gemensamma hushållskostnader'},
    {id:'linked-personal',title:'Personliga levnadskostnader',amount:personal,kind:'Utgift',date:'Månadsvis',note:'Kläder, hygien, transport, fritid och vård'}
  ];
  return <main className="economy-v2">
    <header className="economy-v2-header"><div><p>WORKSPACE · 06</p><h1>Ekonomi</h1><span>En sammanhängande ekonomisk modell för Viktors inkomster, kostnader och beslut.</span></div><button className="family-overview">♧ Familjeöversikt</button></header>
    <section className="economy-balance-card"><p className="economy-kicker">VIKTORS EKONOMISKA ÖVERBLICK</p><h2>{sek(remaining)}</h2><p>Kvar efter samtliga dokumenterade inkomster och kostnader.</p><span className="economy-wallet">▭</span></section>
    <section className="economy-v2-section"><div className="economy-v2-title"><span>01</span><div><h2>Månadens helhet</h2><p>Alla siffror nedan kommer från samma ekonomimodell.</p></div></div><div className="economy-stat-grid"><article className="economy-stat"><span className="economy-stat-icon">▣</span><div><small>Inkomster</small><strong>{sek(income)}</strong><small>Ersättningar + egna tillägg</small></div></article><article className="economy-stat expense"><span className="economy-stat-icon">−</span><div><small>Levnadskostnader</small><strong>-{sek(living)}</strong><small>Budget + egna utgifter</small></div></article><article className="economy-stat balance"><span className="economy-stat-icon">▭</span><div><small>Kvar för Viktor</small><strong>{sek(remaining)}</strong><small>Efter alla kostnader</small></div></article></div><div className="economy-progress-wrap"><div className="economy-progress"><i style={{width:`${retained}%`}}/></div><div className="economy-progress-meta"><span>{retained}% av inkomsten kvar</span><span>{sek(living)} används · {used}%</span></div></div></section>
    <section className="economy-v2-section"><div className="economy-v2-title"><span>02</span><div><h2>Lägg till ekonomisk post</h2><p>Egna tillägg kopplas direkt till samma inkomster, kostnader och kvar-belopp.</p></div></div><button className="economy-action-card" onClick={()=>setFormOpen(value=>!value)}><span>＋</span><span><strong>Ny ekonomisk post</strong><small>Extra inkomst, stöd eller utgift</small></span><span>›</span></button>{formOpen&&<form className="economy-form" onSubmit={add}><div className="economy-form-grid"><label>Rubrik<input name="title" required/></label><label>Belopp<input name="amount" type="number" min="0" required/></label><label>Typ<select name="kind"><option>Inkomst</option><option>Utgift</option><option>Stöd</option></select></label><label>När?<input name="date"/></label></div><label>Förklaring<textarea name="note"/></label><button type="submit">Spara posten</button></form>}</section>
    <section className="economy-v2-section"><div className="economy-v2-title"><span>03</span><div><h2>Sammanlänkade poster</h2><p>Grundposterna redigeras i beslutsunderlaget längre ned. Egna tillägg kan tas bort här.</p></div></div><div className="economy-items">{linkedRows.map(item=><article className="economy-item" key={item.id}><div><h3>{item.title}</h3><p>{item.note} · {item.date}</p></div><strong className={item.kind==='Utgift'?'negative':''}>{item.kind==='Utgift'?'-':'+'}{sek(item.amount)}</strong><span>Sammanlänkad</span></article>)}{model.additional.map(item=><article className="economy-item" key={item.id}><div><h3>{item.title}</h3><p>{item.note} · {item.date}</p></div><strong className={item.kind==='Utgift'?'negative':''}>{item.kind==='Utgift'?'-':'+'}{sek(item.amount)}</strong><button onClick={()=>removeAdditional(item.id)}>Ta bort</button></article>)}</div></section>

    <section className="decision-dashboard" aria-label="Viktors ekonomiska beslutsunderlag">
      <div className="decision-summary-grid"><article><small>INKOMST</small><strong>{sek(income)}</strong><p>Ersättningar, stöd och egna tillägg</p></article><article><small>BERÄKNAD LEVNADSKOSTNAD</small><strong>{sek(living)}</strong><p>Budgeterade och tillagda kostnader</p></article><article><small>BIDRAG TILL HUSHÅLLET</small><strong>{sek(household)}</strong><p>Mat, boende och gemensamma hushållskostnader</p></article><article><small>KVAR FÖR VIKTOR</small><strong>{sek(remaining)}</strong><p>Samma belopp som visas i hela workspacen</p></article></div>
      ...
      <div className="decision-columns"><div>
        ...
        <article className="remaining-card"><small>KVAR EFTER KOSTNADER</small><strong>{sek(remaining)}</strong><p>Detta är samma kvar-belopp som visas högst upp och i sammanfattningen. Ett negativt belopp betyder att kostnaderna överstiger inkomsterna.</p></article>
      </div></div>
