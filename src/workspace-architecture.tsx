...
type MoneyItem={id:string;title:string;amount:number;kind:'Inkomst'|'Utgift'|'Stöd';date:string;note:string;counted?:boolean;outsideBudget?:boolean};
...
function readEconomyModel():EconomyModel{
  ...
  if(existing?.version===3)return{version:3,budget:{...defaultBudget,...existing.budget},additional:Array.isArray(existing.additional)?existing.additional:[]};
  if(existing?.version===2)return{
    version:3,
    budget:{...defaultBudget,...existing.budget},
    additional:(Array.isArray(existing.additional)?existing.additional:[]).map((item:MoneyItem)=>({...item,counted:item.kind==='Utgift'?item.outsideBudget===true:true}))
  };
  if(existing?.version===1)return{
    version:3,
    budget:{...defaultBudget,...existing.budget},
    additional:(Array.isArray(existing.additional)?existing.additional:[]).map((item:MoneyItem)=>({...item,counted:item.kind==='Utgift'?false:true}))
  };
  ...
  const additional=previousItems.filter(item=>!legacyDefaultIds.has(item.id)&&!['Aktivitetsersättning','Boende och vardag','Resestöd'].includes(item.title)).map(item=>({...item,counted:item.kind==='Utgift'?false:true}));
  return{version:3,budget:previousBudget,additional};
}
...
function EconomyWorkspace(){
  ...
  const additionalIncome=model.additional.filter(item=>item.kind!=='Utgift'&&item.counted!==false).reduce((sum,item)=>sum+item.amount,0);
  const customBudgetCosts=model.additional.filter(item=>item.kind==='Utgift'&&item.counted===true);
  const customBudgetTotal=customBudgetCosts.reduce((sum,item)=>sum+item.amount,0);
  const archivedExpenses=model.additional.filter(item=>item.kind==='Utgift'&&item.counted!==true);
  ...
  const livingBudget=household+personal+customBudgetTotal;
  const remaining=income-livingBudget;
  const retained=income>0?Math.max(0,Math.min(100,Math.round(remaining/income*100))):0;
  const used=income>0?Math.round(livingBudget/income*100):0;
  ...
  const addIncome=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=event.currentTarget;const data=new FormData(form);const title=String(data.get('title')||'').trim();if(!title)return;const item:MoneyItem={id:`money-${Date.now()}`,title,amount:Math.max(0,Number(data.get('amount')||0)),kind:String(data.get('kind')||'Inkomst') as MoneyItem['kind'],counted:true,date:String(data.get('date')||''),note:String(data.get('note')||'')};setModel(current=>({...current,additional:[...current.additional,item]}));form.reset();setIncomeFormOpen(false)};
  const addCost=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=event.currentTarget;const data=new FormData(form);const title=String(data.get('title')||'').trim();if(!title)return;const item:MoneyItem={id:`money-${Date.now()}`,title,amount:Math.max(0,Number(data.get('amount')||0)),kind:'Utgift',counted:true,date:String(data.get('date')||'Månadsvis'),note:String(data.get('note')||'Egen rad i levnadsbudgeten')};setModel(current=>({...current,additional:[...current.additional,item]}));form.reset();setCostFormOpen(false)};
  ...
  return <main className="economy-v2">
    ...
    <section className="economy-balance-card"><p className="economy-kicker">VIKTORS EKONOMISKA ÖVERBLICK</p><h2>{sek(remaining)}</h2><p>Inkomst minus den samlade levnadsbudgeten.</p><span className="economy-wallet">▭</span></section>
    <section className="economy-v2-section"><div className="economy-v2-title"><span>01</span><div><h2>Månadens helhet</h2><p>Levnadsbudgeten driver alla kostnadssiffror. Inga separata kostnadslistor konkurrerar med den.</p></div></div><div className="economy-stat-grid"><article className="economy-stat"><span className="economy-stat-icon">▣</span><div><small>Inkomster</small><strong>{sek(income)}</strong><small>Ersättningar + extra inkomster</small></div></article><article className="economy-stat expense"><span className="economy-stat-icon">−</span><div><small>Levnadsbudget</small><strong>-{sek(livingBudget)}</strong><small>Fasta och egna budgetrader</small></div></article><article className="economy-stat balance"><span className="economy-stat-icon">▭</span><div><small>Kvar för Viktor</small><strong>{sek(remaining)}</strong><small>Efter styrande budget</small></div></article></div><div className="economy-progress-wrap"><div className="economy-progress"><i style={{width:`${retained}%`}}/></div><div className="economy-progress-meta"><span>{retained}% av inkomsten kvar</span><span>{sek(livingBudget)} används · {used}%</span></div></div></section>
    <section className="decision-dashboard" aria-label="Viktors ekonomiska beslutsunderlag">
      <div className="decision-summary-grid"><article><small>INKOMST</small><strong>{sek(income)}</strong><p>Ersättningar, stöd och extra inkomster</p></article><article><small>BERÄKNAD LEVNADSKOSTNAD</small><strong>{sek(livingBudget)}</strong><p>Styrs helt av levnadsbudgeten</p></article><article><small>BIDRAG TILL HUSHÅLLET</small><strong>{sek(household)}</strong><p>Mat, boende och gemensamma hushållskostnader</p></article><article><small>KVAR FÖR VIKTOR</small><strong>{sek(remaining)}</strong><p>Samma belopp i hela workspacen</p></article></div>
      ...
      <div className="decision-columns"><div>
        <article className="decision-card"><header><div><small>01 · INKOMST</small><h3>Nuvarande ersättningar</h3><p>Alla inkomster hanteras här och driver samma total.</p></div><span>STYRANDE INKOMST</span></header>{budgetRow('Aktivitetsersättning vid förlängd skolgång','activity','REDIGERBAR')}{budgetRow('Förlängt barnbidrag','childBenefit','REDIGERBAR')}{model.additional.filter(item=>item.kind!=='Utgift'&&item.counted!==false).map(item=><div className="budget-row custom-budget-row" key={item.id}><span><strong>{item.title}</strong><small>{item.note||item.date}</small></span><b>{sek(item.amount)}</b><button type="button" onClick={()=>removeAdditional(item.id)}>Ta bort</button></div>)}<button className="economy-action-card" type="button" onClick={()=>setIncomeFormOpen(value=>!value)}><span>＋</span><span><strong>Lägg till inkomst eller stöd</strong><small>Ingår direkt i total månadsinkomst</small></span><span>›</span></button>{incomeFormOpen&&<form className="economy-form embedded-economy-form" onSubmit={addIncome}><div className="economy-form-grid"><label>Rubrik<input name="title" required/></label><label>Belopp<input name="amount" type="number" min="0" required/></label><label>Typ<select name="kind"><option>Inkomst</option><option>Stöd</option></select></label><label>När?<input name="date"/></label></div><label>Förklaring<textarea name="note"/></label><button type="submit">Spara inkomsten</button></form>}<footer><strong>Total månadsinkomst</strong><b>{sek(income)}</b></footer></article>
        <article className="decision-card"><header><div><small>02 · VIKTORS FAKTISKA KOSTNADER</small><h3>Levnadsbudget</h3><p>Detta är den enda auktoritativa kostnadskällan. Lägg även till egna kostnadsrader här.</p></div><span>STYRANDE MODELL</span></header>{budgetRow('Mat','food','KOV 2026')}{budgetRow('Boendebidrag / hyresandel','housing','BEDÖMNING')}{budgetRow('El, vatten, internet, tv','utilities','BEDÖMNING')}{budgetRow('Kläder och skor','clothes','BEDÖMNING')}{budgetRow('Hygien och förbrukning','hygiene','BEDÖMNING')}{budgetRow('Transport och resor','transport','BEDÖMNING')}{budgetRow('Fritid och aktiviteter','leisure','BEDÖMNING')}{budgetRow('Mediciner / vård / övrigt','care','BEDÖMNING')}{customBudgetCosts.map(item=><div className="budget-row custom-budget-row" key={item.id}><span><strong>{item.title}</strong><small>{item.note||item.date}</small></span><b>{sek(item.amount)}</b><button type="button" onClick={()=>removeAdditional(item.id)}>Ta bort</button></div>)}<button className="economy-action-card" type="button" onClick={()=>setCostFormOpen(value=>!value)}><span>＋</span><span><strong>Lägg till kostnadsrad</strong><small>Blir en del av levnadsbudgeten och uppdaterar allt</small></span><span>›</span></button>{costFormOpen&&<form className="economy-form embedded-economy-form" onSubmit={addCost}><div className="economy-form-grid"><label>Rubrik<input name="title" required/></label><label>Belopp<input name="amount" type="number" min="0" required/></label><label>När?<input name="date" placeholder="Månadsvis"/></label><label>Underlag/status<input name="note" placeholder="Bedömning, kvitto eller beslut"/></label></div><button type="submit">Lägg till i levnadsbudgeten</button></form>}<footer><strong>Total levnadsbudget</strong><b>{sek(livingBudget)}</b></footer>{archivedExpenses.length>0&&<details className="archived-economy-items"><summary>Tidigare dokumenterade kostnadsposter ({archivedExpenses.length})</summary>{archivedExpenses.map(item=><div key={item.id}><span>{item.title} · {sek(item.amount)}</span><button type="button" onClick={()=>removeAdditional(item.id)}>Ta bort</button></div>)}</details>}<div className="budget-actions"><button onClick={()=>localStorage.setItem(modelKey,JSON.stringify(model))}>Spara lokalt</button><button className="secondary" onClick={()=>setModel(current=>({...current,budget:defaultBudget}))}>Återställ startvärden</button></div></article>
        <article className="decision-card distribution"><header><div><small>03 · FÖRDELNING</small><h3>Vad går till hushållet?</h3></div><span>SPÅRBAR</span></header><div className="distribution-grid"><div><small>HUSHÅLLSBIDRAG</small><strong>{sek(household)}</strong><p>Mat + boende + gemensamma hushållskostnader</p></div><div><small>PERSONLIGA KOSTNADER</small><strong>{sek(personal+customBudgetTotal)}</strong><p>Personliga standardrader + egna kostnadsrader</p></div></div><small>ANDEL AV VIKTORS INKOMST SOM ANVÄNDS</small><div className="decision-progress"><i style={{width:`${Math.min(100,used)}%`}}/></div><p>{sek(livingBudget)} av {sek(income)} · {used}%</p></article>
      </div><div>
